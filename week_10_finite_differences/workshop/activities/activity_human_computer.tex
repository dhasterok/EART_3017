\begin{activity}{Human Finite Difference Solver}
...
\begin{itemize}
  \item Apply the finite difference update rule by computing temperature from neighboring values.
...
This means:
\begin{itemize}
    \item you look at your two neighbors,
...
\begin{questions}
    ...
    \begin{enumerate}
        \item Show your value to your neighbors.
    ...
    \begin{itemize}
        ...
        \item What role do neighboring nodes play?
...
\begin{itemize}
    ...
    \item What steady-state behavior emerges?
\end{itemize}
...
\textbf{Numerical models evolve through local interactions.} Each node only communicates with its immediate neighbors. Global behavior (e.g., a uniform steady state) emerges from many local interactions over many time steps. There is no global solution found in one step.

\textbf{Information propagates at a finite rate.} Because each update depends only on nearest neighbors, a temperature change at one end of the chain cannot instantaneously affect the other end. It must propagate node by node, step by step. This is the discrete analogue of finite diffusion speed in the continuum.

\textbf{Steady state is reached when updates become zero.} The system reaches a steady state when every node's temperature equals the average of its neighbors: $T_i = (T_{i-1} + T_{i+1})/2$. This is the discrete form of $\nabla^2 T = 0$ --- the Laplace equation for heat without sources.
