...
\begin{questions}
    ...
    \begin{tikzpicture}[scale=0.9]
            ...
            \draw[<-, thick] (\w/2,-4.6) -- (\w/2,-5.1) node[below] {$q_b$};
    \end{tikzpicture}
    \end{center}
    ...
    \end{equation}
    This function is the thickness-weighted \emph{harmonic mean}, the same result as for two layers. The result falls out of the derivation for arbitrary $N$ with no extra work, since the sum in (d) was already general.

    \question{Parallel Case: Heat Flow Along the Layering}

    Now consider the \emph{same} stack of layers, but with heat flowing along the layering instead, parallel to strike, not across it. Each layer independently carries its own share of the heat flow, $q_i$ between two shared end temperatures $T_A$ and $T_B$.

    \begin{center}
    \begin{tikzpicture}[scale=0.9]
        \def\w{4.5}
        \draw[thick] (-0.2,0) -- (\w+0.2,0);
        \draw[thick] (-0.2,-1) -- (\w+0.2,-1);
        \draw[thick] (-0.2,-2) -- (\w+0.2,-2);
        \draw[thick] (-0.2,-3.6) -- (\w+0.2,-3.6);
        \draw[thick] (-0.2,-4.6) -- (\w+0.2,-4.6);
        ...
        \draw[thick,dashed,gray!60] (0,-4.6) -- (0,0) node[above,black] {$T_A$};
        \draw[thick,dashed,gray!60] (\w,-4.6) -- (\w,0) node[above,black] {$T_B$};

        \draw[<-, thick] (-0.1,-0.5) -- (-0.9,-0.5) node[left] {$q_1$};
        \draw[<-, thick] (-0.1,-1.5) -- (-0.9,-1.5) node[left] {$q_2$};
        \draw[<-, thick] (-0.1,-4.1) -- (-0.9,-4.1) node[left] {$q_N$};
    \end{tikzpicture}
    \end{center}
...
\definecolor{grass}{RGB}{85,145,60}

\newcommand{\slow}[2]{
    \draw[fill,orange] (#1,#2) circle (0.5) node[white,font=\bfseries] {\scriptsize SLOW};
    \draw[thick,white] (#1,#2) circle (0.45);
}
\newcommand{\fast}[2]{
    \draw[fill,grass] (#1,#2) circle (0.5) node[white, font=\bfseries] {\scriptsize GO};
    \draw[thick,white] (#1,#2) circle (0.45);
}

\begin{center}
    \begin{tikzpicture}
    % Figure -- layers are 3x the original thickness so the SLOW/GO
    % markers (radius 0.5) no longer overlap; band width is widened to
    % match, keeping the overall figure roughly square.
    \fill[gray!60] (0,0) rectangle (6.0,1.2);
    \slow{7.5}{0.6}
    \fill[gray!20] (0,1.2) rectangle (6.0,2.4);
    \fast{7.5}{1.8}
    \fill[gray!60] (0,2.4) rectangle (6.0,3.6);
    \slow{7.5}{3.0}
    \fill[gray!20] (0,3.6) rectangle (6.0,4.8);
    \fast{7.5}{4.2}
    \fill[gray!60] (0,4.8) rectangle (6.0,6.0);
    \slow{7.5}{5.4}

    \draw[->,thin,blue] (6.5,0.6) -- (-0.5,0.6);
    \draw[->,thick,blue] (6.5,1.8) -- (-0.5,1.8);
    \draw[->,thin,blue] (6.5,3.0) -- (-0.5,3.0) node[align=center,rotate=90,above] {parallel\\arithmetic};
    \draw[->,thick,blue] (6.5,4.2) -- (-0.5,4.2);
    \draw[->,thin,blue] (6.5,5.4) -- (-0.5,5.4);

    \draw[->,red] (1,-0.5) -- (1,6.5);
    \draw[->,red] (3,-0.5) -- (3,6.5) node[align=center,above] {perpendicular\\harmonic};
    \draw[->,red] (5,-0.5) -- (5,6.5);
