...
The practical notebook (\texttt{week\_7\_fourier.ipynb}) is organized into six
sections:
\begin{enumerate}
...
Narrow, localized anomalies disappear first because they correspond to high
...
\paragraph{Expected answer.}
The residual (observed $-$ regional) emphasizes sources not accounted for by the
...
(e.g.\ mafic intrusions), but may disagree where remanent magnetization decouples
...
\begin{enumerate}[label=\alph*.]
  ...
  \item Near the equator ($I \approx 0$), the direction-cosine factor $\Theta$
        ...
        regularization, so RTP cannot be reliably applied at low magnetic
        latitudes.
\end{enumerate}
...
\subsection*{Q13 — Remanent magnetization}

\paragraph{Expected answer.}
The RTP filter applies a single phase correction based on the ambient-field
direction.  Where a body carries remanent magnetization in a significantly
different direction (e.g.\ a reversely magnetized unit), RTP will
