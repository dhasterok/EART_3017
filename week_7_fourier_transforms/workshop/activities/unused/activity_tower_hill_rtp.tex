\begin{activity}{Tower Hill Case Study}

\section{Purpose}

This case study applies the Fourier-domain magnetic processing methods developed in the RTP demo notebook to real aeromagnetic data from the Tower Hill Volcanic Complex in south-western Victoria, Australia.
...
Tower Hill ($38.3^\circ$\,S, $142.4^\circ$\,E) is a Quaternary maar complex
...
encountered groundwater, generating a series of phreatic explosions.
...
filled with highly fragmented, magnetized basaltic material.

\subsection{Why a maar produces a magnetic anomaly}

Mafic magmas are typically rich in magnetite, which becomes permanently
magnetized below the Curie temperature ($\sim$580\,$^{\circ}$C for magnetite)
as the rock cools.
The diatreme fill and associated intrusive material therefore carry both
induced and remanent magnetization, producing a measurable TMI anomaly
...
only anomalies due to crustal magnetization contrasts.
...
field is vertical.
The anomaly of a compact, uniformly magnetized body is then symmetric: a
...
extreme: the positive anomaly maximum is displaced a few kilometers
...
The total-field magnetic anomaly of a thin magnetized sheet at depth $z$
...
and $\varepsilon$ is a small water-level stabilization constant
...
Even with water-level stabilization, the filter amplifies these components
strongly, producing ringing artifacts and unreliable phase corrections.

Students exploring the inclination slider in the notebook should observe that
artifacts become visible below about $|I| \approx 15$--$20^\circ$.
...
Both the ambient field direction and the assumed magnetization direction
are set to the IGRF values at the site:
$I = -70^\circ$, $D = +10.5^\circ$.
This is the standard assumption of purely induced magnetization, which is
a reasonable first approximation for Quaternary basalts.
Where remanent magnetization is significant and differs in direction
...
strongly magnetized basaltic body.
...
to one kilometer visible in the maps.
...
The standard RTP assumes induced magnetization parallel to the present-day
field.
Remanently magnetized rocks carry a frozen-in direction from the time of
cooling.
...
the magnetization direction can be incorporated explicitly.
...
\subsection{Heterogeneous magnetization}
...
\begin{itemize}
  ...
  \item Remanent magnetization, heterogeneous susceptibility, and
