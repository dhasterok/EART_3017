...
\begin{itemize}
  \item Explain why aliasing produces an internally consistent but incorrect interpretation, rather than an obviously flawed dataset.
  \item State the Nyquist relationship between station spacing, sampling frequency, and the longest wavelength that can be trusted.
  \item Identify what, if anything, in a survey's own data or design should raise suspicion of aliasing before it can be confirmed by resurveying.
  \item Choose a station spacing that resolves a target wavelength, given an estimate of the shallowest source depth of interest.
\end{itemize}
...
\ntextbox{Aliasing}{
    ...
    \begin{center}
        ...
        \captionof{figure}{Demonstration of frequency folding as a result of aliasing.  The coarse sampling results in aliasing whereas the dense sampling does not.  The Nyquist frequencies $f_{N}$ are the theoretical limits based on each survey's sampling interval.}
    \end{center}
}{core}
...
\textbf{Aliasing produces a wrong answer that looks exactly as trustworthy as a right one.} Both surveys in this activity produced a smooth curve that fit every station exactly; nothing about the coarse curve looked wrong until it was checked against denser data. The only defense is choosing $\Delta x$ correctly \emph{before} you go to the field.

\textbf{Aliasing can be difficult to detect.} A single, evenly-spaced dataset cannot tell you on its own whether it is under-sampled --- the aliased wavelength is just as smooth, regular, and well-fit as the true one would have been. Catching it generally requires an independent check (denser infill sampling, a different station geometry, or prior knowledge of the expected wavelength range), not more careful analysis of the same data.

\textbf{Aliasing can be an expensive or unrecoverable error.} Correcting it after the fact means returning to the field to resurvey at finer spacing, which costs time and money and may not even be logistically possible. If a decision, e.g., where to drill, where to mine, how to interpret regional structure, has already been made based on the aliased wavelength then the error may be effectively unrecoverable.  For temporal sampled problems, the only remedy is to repeat the experiment---if that is even possible. These potential aliasing error is why the Nyquist criterion belongs in survey design, before data are collected, not in interpretation after the fact.
