...
\begin{questions}
    ...
    \question{When RTP Breaks Down: Instability and Limitations}
        ...
        The direction-cosine factor $\Upsilon$ approaches zero for wavenumber components aligned with the horizontal field direction as $I\to0^\circ$. Even with water-level stabilization, the filter then amplifies those components strongly, producing ringing artefacts and unreliable phase corrections. The figure below reruns the exact same pipeline as Question 4, at the same declination, but at $I=8^\circ$ -- well below the stability threshold.
