...
# Static figures for Activity 7.E (activity_nevada_fft.tex) are saved here.
# Filenames below are referenced directly by \includegraphics in the .tex --
# if you rename anything here, update the .tex to match.
...
def _savefig(fig, name):
    ...
    fig.savefig(FIGDIR / f"{name}.pdf", dpi=200, bbox_inches="tight")
...
def plot_gravity_profiles(x_km, y_km, datasets, y_targets=(4600, 4700),
    ...
    If savename is given, the figure is written to FIGDIR/savename via
    _savefig before it is shown.
    """
    n = len(y_targets)
...
plt.grid(False)
plt.tight_layout()
_savefig(fig, "01_nevada_bouguer_map")
plt.show()
...
def show_aliasing(spacing_km=2, savename=None):
...
# %%
# Static snapshots for the printed activity handout (Activity 7.E).
# Two representative spacings: full resolution (resolved) and 16 km
# (aliased -- Nyquist = 32 km exceeds the 20-60 km basin band, matching
# the key-observations text above).
show_aliasing(spacing_km=2,  savename="02a_aliasing_realdata_2km")
show_aliasing(spacing_km=16, savename="02b_aliasing_realdata_16km")
...
ax.loglog(wavelength_ew, power_ew, 'red',  lw=0.9, label='E–W')
ax.loglog(wavelength_ns, power_ns, 'blue', lw=0.9, label='N–S', alpha=0.7)
...
plt.grid(False)
plt.tight_layout()
_savefig(fig, "04_power_spectra_ew_ns")
plt.show()
...
# %% [markdown]
# ### 2.3 Power-law background of the spectrum
#
# **Objective:** separate the smooth red-noise background (expected for any
# potential field) from the elevated power that marks the basin-scale signal,
# by fitting a power law to wavelength ranges away from the basin band.
#
# The power-law is anchored at 6.2-10 km and 80-120 km -- both well outside
# the 20-60 km basin band -- so the fit describes the background trend without
# being pulled toward the signal we're trying to isolate.
...
plt.grid(False)
plt.tight_layout()
_savefig(fig, "05_power_spectra_background")
plt.show()

# %% [markdown]
# > **Key observations:**
# > - Both spectra sit above the power-law background specifically in the
# >   **20-60 km** band -- this excess above the smooth red-noise trend is
# >   the basin signal, not an artefact of the fit.
# > - The E–W spectrum's excess peaks nearer 20-25 km; the N–S spectrum's
# >   excess sits at longer wavelengths, consistent with basins that are
# >   elongated N–S.
...
plt.grid(False)
plt.tight_layout()
_savefig(fig, "06_highpass_filtered_map")
plt.show()
...
plt.grid(False)
plt.tight_layout()
_savefig(fig, "08_sediment_thickness_map")
plt.show()
...
ax.hist(h_km_valid, bins=bins, color='C2', edgecolor='none')
...
plt.grid(False)
plt.tight_layout()
_savefig(fig, "09_sediment_thickness_distribution")
plt.show()
...
# >   distribution, not the bulk of the map — they are most likely a
# >   leftover-regional-signal artefact (some crustal-thickness-scale power
# >   leaks through the 60-100 km transition band of the high-pass filter and
# >   is misread by the inversion as extra sediment), concentrated in a few
...
def show_pipeline_aliasing(decimation=1, savename=None):
    step = int(decimation)
...
# %%
# Static snapshots for the printed activity handout (Activity 7.E).
# decimation=1 (2 km, resolved) vs decimation=8 (16 km, Nyquist=32 km,
# clearly aliased -- matches the "Decimation >= 7" case in the
# key-observations text above).
show_pipeline_aliasing(decimation=1, savename="10a_pipeline_aliasing_resolved")
show_pipeline_aliasing(decimation=8, savename="10b_pipeline_aliasing_aliased")
