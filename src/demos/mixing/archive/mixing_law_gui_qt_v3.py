# -*- coding: utf-8 -*-
"""
Mixing-Law Transport GUI (PyQt6) — Layering V3 (Dock + Geologic labels)
-----------------------------------------------------------------------
• Intuitive layering controls: A/B bed counts (thick↔thin), Thickness variability (CV) and distribution,
  Contact half-width (erf), Continuity (probability & gap length), and Fold amplitude/wavelength.
• 3rd component C with affinities (C→A, C→B), attach fraction, threshold, optional C-only layers.
• Series (layers ⟂ flow) built directly; Parallel (lanes ∥ flow) via 90° rotation of the series field.
• Random granular mode kept (corr_len + ln k noise).
• Dijkstra with uniform-random start (left) + free exit (right), stable colorbar, animation point fix.

Run:  python mixing_law_gui_qt_v3.py
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Mixing-Law Transport (PyQt6) — Layering V3")
    ...
    def _build_ui(self):
        ...
        dock_container = QWidget(); rv = QVBoxLayout(dock_container); dock.setWidget(dock_container)
        ...
        # Layering controls (Series built; Parallel by rotation)
        ...
        self.sp_NA.setToolTip("Minimum number of A beds/bands; more beds ⇒ thinner")
        ...
        self.sp_NB.setToolTip("Minimum number of B beds/bands; more beds ⇒ thinner")
        ...
        self.sp_Lme = QSpinBox(); self.sp_Lme.setRange(1, 999999); self.sp_Lme.setValue(self.L_meander)
        ...
        # Components
        g_comp = QGroupBox("Components (A, B, C)")
        ...
        self.sp_phiCB = QDoubleSpinBox(); self.sp_phiCB.setRange(0.0,1.0); self.sp_phiCB.setSingleStep(0.05); self.sp_phiCB.setValue(self.phi_CB)
        self.sp_eta = QDoubleSpinBox(); self.sp_eta.setRange(0.0,1.0); self.sp_eta.setSingleStep(0.05); self.sp_eta.setValue(self.eta_attach)
        self.sp_tau = QDoubleSpinBox(); self.sp_tau.setRange(0.0,1.0); self.sp_tau.setSingleStep(0.05); self.sp_tau.setValue(self.tau_aff)
        self.sp_NCmin = QSpinBox(); self.sp_NCmin.setRange(0,50); self.sp_NCmin.setValue(self.N_C_min)
        for w, tip in [
            (self.sp_phiCA, "Affinity of C to sit within A layers (0--1)"),
            (self.sp_phiCB, "Affinity of C to sit within B layers (0--1)"),
            (self.sp_eta,   "Fraction of C that tries to attach inside A/B; rest forms C-only layers"),
            (self.sp_tau,   "If both affinities below this threshold, C prefers its own layers"),
            (self.sp_NCmin, "Minimum number of C-only layers if present")
        ]:
            w.valueChanged.connect(self._on_params); w.setToolTip(tip)
        af.addRow("Affinity C→A:", self.sp_phiCA)
        af.addRow("Affinity C→B:", self.sp_phiCB)
        af.addRow("Attach fraction:", self.sp_eta)
        af.addRow("Affinity threshold:", self.sp_tau)
        af.addRow("Min C-only layers:", self.sp_NCmin)
        rv.addWidget(g_aff)

        # Simulation & prediction
        g_sim = QGroupBox("Simulation & Prediction")
    ...
    def _on_params(self, *args):
        self.corr_len = self.sp_corr_len.value(); self.sigma_ln_noise = self.sp_sigma_ln.value()
