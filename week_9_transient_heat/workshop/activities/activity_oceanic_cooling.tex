...
Write the half-space predictions as $q(t) = c_q/\sqrt{t}$ and $s(t) = c_s\sqrt{t}$, absorbing all the physical constants ($k$, $\kappa$, $T_m-T_0$, $\rho_m$, $\alpha$, \ldots) into the single constants $c_q$ and $c_s$. Two real data points from each dataset are given below; $s$ is depth relative to the ridge-depth reference of 2500~m.
