...
\input{numerical/fields_and_properties}
...
\begin{figure}[bth]
    ...
    Solutions of $x = y^{1/2}$ are multivalued unless constrained.}
    \label{fig:nonunique}
\end{figure}
...
\end{equation}
where $\lambda$ is the regularization hyperparameter that controls the trade-off between fitting the data and satisfying the model constraints. The choice of $\phi_m$ depends on the desired characteristics of the model.
...
\begin{table}{htbp}
    \centering
    \caption{Comparison of Levenberg--Marquardt and steepest descent parameters.}
