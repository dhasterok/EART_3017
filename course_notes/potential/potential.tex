...
Substituting the constitutive relationship into conservation:
\begin{equation}
...
\input{potential/potential_sources}
...
To make the vertical behavior clearer, we temporarily introduce a height coordinate $h$, defined positive upward---opposite to our usual geophysical convention where $z$ positive down. In terms of $h$, Laplace's equation implies
\[
...
% mathematically, or hidden within other variables.  But we need the test mass to
...
% gravitational field it takes energy (work) to move it (we gain potential).  If we
