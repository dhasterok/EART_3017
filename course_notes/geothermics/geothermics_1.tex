...
\ntextbox{Shear heating in the Earth}{
    ...
    \input{geothermics/strain_rates}
...
\input{geothermics/thermal_conductivity_minerals} % tab:tcmin
...
\input{geothermics/rad_isotope_table}
...
\begin{figure}[hbtp]
    ...
    \label{fig:hp_age}
\end{figure}
\input{geothermics/hpe_table.tex}
...
\ntextbox{Time Dependence of Radiogenic Heat Production}{
    ...
    \]
    where $X_{i,j}$ is the abundance of isotope $i$ in element $j$.
...
\end{itemize}
The monopole arises from the effective source contrast $\Delta$, which couples both heat production \emph{and} conductivity differences — a sphere with high heat production but proportionally high conductivity can have a suppressed monopole.  This behavior is directly analogous to potential field problems, where source contrasts produce monopoles and property contrasts in a background field produce dipoles.
