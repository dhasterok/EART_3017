...
Something you are undoubtedly familiar with is traveling.  Traveling from home
...
\vspace{8pt}
\noindent {\it How fast was I traveling?}
...
\begin{figure}[hbt]
    ...
    \caption{Average velocity on the bike ride between home and work.}
    \label{fig:vavg1}
\end{figure}
...
Thus far, we have assumed by trip between home and work is in 1-dimension.
...
and interfaces---or discrete changes in behavior with time---between
...
\begin{align*}
    ...
    && &= 2 x.
\end{align*}
Now, we multiply each together and substitute each composite function to obtain
the derivative,
\begin{align*}
...
bounds, $x_i$ to $x_{i+1}$, because as the interval width approaches zero the
value of the function on each side of the interval approach the same value.
...
column width.  The $x$ coordinates that the function is evaluated at are $a$, $a
...
integration and back substitute the $v(x)$.
\begin{align*}
...
\end{equation}
where $\dd{\vb{a}}$ implies area infinitesimals and for volume,
\begin{equation}
    \iiint f(x,y,z) \dd{x} \dd{y} \dd{z} = \int_{V} f(x,y,z) \cdot \dd{\vb{v}},
\end{equation}
where $\dd{\vb{v}}$ implies volume infinitesimals.  Note that the order in which
