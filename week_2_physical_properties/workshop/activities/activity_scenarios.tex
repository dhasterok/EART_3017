...
\begin{activity}{When to Estimate Physical Properties}

\section*{Purpose}

This activity develops professional judgement in selecting physical property estimation methods for realistic geoscience problems. Through a series of scenarios drawn from basin analysis, tectonics, geodynamics, mineral exploration, and geothermal studies, you will practice identifying the most appropriate method given the available data, the target property, and the geological context.

\section*{Learning Outcomes}

By the end of this activity, you should be able to:
\begin{itemize}
    \item Compare the strengths and limitations of different approaches to estimating physical properties (direct measurement, lab data, composition-based estimates, proxies/correlations, geophysical inversion).
    \item Select and justify an estimation method appropriate to the available data, the scale of the problem, and the target property.
    \item Formulate targeted geological questions that would reduce uncertainty in a physical property model.
    \item Identify sources of non-uniqueness and uncertainty in property estimation, including pressure--temperature effects, alteration, and limited sample coverage.
\end{itemize}
...
The responses to these scenarios closely align with the lecture notes on methods used to determine/estimate physical properties.
...
\textbf{No single method suits every problem.} Direct measurement on samples is most accurate but requires access; composition-based estimates are portable but depend on assumed mixing laws; geophysical inversion is non-unique but provides spatial coverage. The right choice depends on the data available, the scale of the model, and the required accuracy.

\textbf{Geological questions constrain physical property models.} Before selecting a method, a geophysicist must understand the geological context: What rock types are present? What are their pressure--temperature conditions? Is alteration or weathering likely? How well sampled is the region? The answers narrow the method options significantly.

\textbf{Non-uniqueness is unavoidable.} Different rock types or mixing geometries can produce similar effective properties. Uncertainty should be quantified and propagated into the final geophysical model, not treated as a source of error to be hidden.
