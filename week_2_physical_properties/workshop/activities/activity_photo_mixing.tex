...
\begin{itemize}
  \item Identify the fabric or geometry of a rock from a photograph and classify it as random granular or layered. 
...
\begin{tblr}{
  ...
  & & \textbf{Scale} & \SetCell[c=2]{c}{\textbf{Mixing Law}} & \\
  & & & \SetCell[c=2]{c}{(\textbf{A})rithmetic, (\textbf{G})eometric, (\textbf{H})armonic} & \\
  \textbf{Photo} &
  \textbf{Description} &
  ...
  \textbf{Ex.} & uniform porous sandstone & Meso & A & G & I \\
  1. & & & & & \\
  2. & & & & & \\
  3. & & & & & \\
  4. & & & & & \\
...
\begin{minipage}{0.30\textwidth}
  ...
  \begin{tikzpicture}
    \path[use as bounding box] (-1,-2) rectangle (4,5);
    % Title
    \node[font=\bfseries] at (1.5,4.5) {Parallel Flow};
...
\begin{minipage}{0.30\textwidth}
  ...
  \begin{tikzpicture}
    \path[use as bounding box] (-1,-2) rectangle (4,5);
    % Title
    \node[font=\bfseries] at (1.5,4.5) {Perpendicular Flow};
    ...
    % Description
    \node[align=center] at (1.5,-0.2)
        {Bottlenecks\\when perpendicular};
