...
\begin{itemize}
  ...
  \item create a common language across geology and geophysics (e.g., “sharp, continuous series layers will drive harmonic behavior across bedding”).
\end{itemize}
...
Parallel (lanes $\parallel$ flow) $\Rightarrow$ \textbf{arithmetic} (fastest);\\
Random granular $\Rightarrow$ \textbf{geometric} (middle);\\
Series (layers $\perp$ flow) $\Rightarrow$ \textbf{harmonic} (slowest).
\end{tcolorbox}
...
\paragraph{Layer counts and thickness.} For series--oriented layering (layers $\perp$ transport), specify integers $(N_A,N_B)$ (and optionally $N_C$). Draw raw thicknesses for each set,
\[
    t_i \sim
    \begin{cases}
...
\paragraph{Parallel orientation.} Lanes $\parallel$ transport are obtained by rotating the constructed series field by $90^\circ$ (or by swapping $x$/$y$ roles during construction).
...
\[
    \text{parallel (arithmetic) fastest} \;<\; \text{random granular (geometric)} \;<\; \text{series (harmonic) slowest}.
\]
...
For \emph{series} layers (layers $\perp$ transport), with layer thicknesses $t_i$ and within--layer conductivities $k_i$,
\[
    \frac{1}{k_{\text{eff}}^{\text{series}}} \;=\; \sum_i \frac{t_i}{H}\,\frac{1}{k_i}.
\]
For \emph{parallel} lanes (lanes $\parallel$ transport), with lane widths $w_i$,
\[
    k_{\text{eff}}^{\text{parallel}} \;=\; \sum_i \frac{w_i}{W}\,k_i.
\]
For \emph{random granular} mixtures with lognormal variability, $k_{\text{eff}}$ often trends toward the \emph{geometric mean} at the representative elementary volume. Diffuse contacts shift series behavior toward geometric; leaky layers (continuity $<1$) also move harmonic toward geometric.
...
\begin{enumerate}
  ...
  \item Use \textbf{harmonic} across series layers or \textbf{arithmetic} across parallel lanes to estimate $k_{\text{eff}}$.
...
\begin{itemize}
  ...
  \item \textbf{Series (harmonic)} is the \emph{slowest} if layers are continuous and contacts are sharp. Diffuse contacts ($w_{\text{int}}>0$) and leaks ($p_{\text{cont}}<1$) move behavior toward geometric.
...
\begin{itemize}
  \item \textbf{Series/Parallel via rotation:} Build series--oriented layers once, then rotate by $90^\circ$ to obtain parallel lanes with identical parameter semantics.
...
\begin{enumerate}
  \item \textbf{Geometry governs transport}: parallel (arithmetic) $<$ granular (geometric) $<$ series (harmonic).
  \item \textbf{Interfaces matter}: sharp, continuous barriers drive harmonic limits; diffuse or leaky contacts weaken series behavior.
...
Choose two or three components (e.g., A = quartz--feldspar, B = mica-rich, C = mafic). Pick area fractions $f_A, f_B, f_C$ with $f_A+f_B(+f_C)=1$. For layers $\perp$ flow (series geometry), set minimum counts $N_A\!\ge\!1$, $N_B\!\ge\!2$ (and $N_C$ if C has its \emph{own} layers). More layers $\Rightarrow$ thinner beds.
...
We assign a \emph{continuity probability} $p_{\text{cont}}$. If a layer is \emph{non-continuous} ($1-p_{\text{cont}}$), we insert small gap segments with mean length $L_{\text{gap}}$. These \emph{leaks} let some flow slip around barriers, shifting \emph{series} behavior toward \emph{geometric}.
...
\item \emph{Series layers} (barriers perpendicular to flow): must cross low-$k$ layers in series $\Rightarrow$ \textbf{harmonic} across the stack.
...
\item \textbf{Leaky layers ($p_{\text{cont}}<1$)}: provide shortcuts around barriers $\Rightarrow$ again push series behavior toward geometric.
\end{itemize}
...
\[
t_{\text{parallel}} < t_{\text{granular}} < t_{\text{series}}.
\]
...
\begin{itemize}[leftmargin=*]
\item \textbf{Series (layers $\perp$ flow)}: $k_{\text{eff}}^{-1} = \sum_i (t_i/H)\,k_i^{-1}$ (\emph{harmonic}). Diffuse/ leaky layers move toward \emph{geometric}.
...
\begin{enumerate}[leftmargin=*]
\item \textbf{Choose geometry:} Series (layers $\perp$ flow), Parallel (lanes $\parallel$ flow), or Random (granular).\\
\emph{Sedimentary/foliated strata $\to$ Series; channelized/lineated fabrics $\to$ Parallel; massive equant $\to$ Random.}
...
\item \textbf{Across-layer mix}: use \emph{harmonic} for series or \emph{arithmetic} for parallel to estimate $k_{\text{eff}}$.
...
\begin{enumerate}[leftmargin=*]
\item \textbf{Geometry governs}: Parallel $<$ Granular $<$ Series for traversal time.
