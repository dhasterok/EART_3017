\newcommand\bax{
  \begin{center}
    \begin{tikzpicture}[scale=0.85]
      ...
      \draw[->,red] (-5,-1.1) -- (-5,1.1) node[left] {$B_z$};
      \draw[thin] (-5,0) -- (5,0);
      \draw[->,blue] (5,-1.1) -- (5,1.1) node[right] {$g_z$};
    \end{tikzpicture}
  \end{center}
}
...
\begin{itemize}
  ...
  \item Induced magnetization only
...
For each structure:
\begin{enumerate}
  \item Draw the gravity anomaly
  \item Sketch the expected vertical magnetic field associated with a vertical magnetization
  \item Indicate sign, symmetry, width, and strongest gradient
\end{enumerate}
...
\begin{questions}
  ...
  \begin{enumerate}
    \item What are the major differences between the magnetic and gravity anomalies?
    \answerbox[2cm]
    \item How would the anomaly for the sphere and finite horizontal slab change if the magnetization were horizontal rather than vertically oriented?  Remember, we are still looking at $B_z$.
    \answerbox[3cm]
    \item How do the differences between gravity and magnetic anomalies reduce the non-uniqueness of geologic interpretations?
    \answerbox[3cm]
  \end{enumerate}
...
\textbf{Sign and shape depend on source geometry and field direction, not just on source strength.} A mass excess always produces a positive gravity anomaly, full stop. A susceptibility contrast produces a magnetic anomaly whose sign and shape depend on the body's geometry \emph{and} on the direction of the inducing field --- at the magnetic pole a symmetric body gives a symmetric, bipolar anomaly; at lower latitudes the same body gives an asymmetric one with shifted lobes. Gravity has no equivalent to either effect. This traces back to the same monopole-versus-dipole distinction that gives magnetic fields their more complex geometry: a mass excess has only one sign, but every magnetic source carries both a north and a south pole.
...
\textbf{Non-uniqueness is greater in magnetics than gravity.} Three unknowns --- susceptibility, geometry, and magnetization direction --- must all be constrained simultaneously. Remanent magnetization (a frozen-in field direction different from today's) adds a fourth unknown. Additional constraints from geology, paleomagnetism, or other data are usually required to discriminate between models.
