...
Consider a reference Earth model with a constant crustal thickness $T_0$. Surface topography $h(x)$ is compensated by a crustal root $r(x)$ such that columns of equal width have equal mass above a compensation depth. This is the general isostatic mass-balance principle, applied here to a single topographic column compared against a flat reference column.
...
\begin{questions}
    ...
    \begin{enumerate}[label=\roman*]
        \item Using the general isostatic mass-balance principle, write the pressure balance between the reference column ($T_0$, $\rho_c$, mantle below) and the topographic column ($T_0+h+r$, $\rho_c$), evaluated at the depth of compensation. Show that this reduces to
        \[
...
\textbf{Under perfect compensation, topography and its root cancel exactly.} The gravity signal from a mountain ($+2\pi G \Delta \rho_h h$) is exactly cancelled by its deeper crustal root ($-2\pi G \Delta \rho_r r$). A perfectly compensated terrain therefore has no isostatic gravity anomaly --- any residual signal indicates incomplete compensation or lateral density variations.
...
\begin{tikzpicture}[scale=1.1]
    ...
    % Reference surface (sea level)
    \draw[dashed] (-4,0) -- (4,0) node[midway,above] {$+2\pi G \Delta\rho_h h$};

    % Moho (reference crustal thickness)
    \draw[dashed] (-4,-\Tzero) -- (4,-\Tzero) node[midway,below] {$-2\pi G \Delta\rho_r r$};
    ...
    % Label T0 with bracket
    \draw[<->,gray] (0,0) -- (0,-\Tzero) node[midway,left] {cancels};
