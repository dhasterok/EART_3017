...
\begin{itemize}
    \item Explain the difference between forward modelling, grid search, and gradient-based inversion.
    ...
    \item Explain how model parameterization can improve or degrade inversion performance.
    \item Describe the influence of noise on inversion results and uncertainty.
\end{itemize}

\ntextbox{Theory}{
    \textbf{Gravity of a Buried Sphere}
    ...
    \]
    where $\vb{g}_{\mathrm{obs}}$ are the observed gravity data, $g(\vb{m})$ is the predicted gravity using Equation~\ref{eq:forward}, $\vb{W}_d$ is a diagonal matrix of inverse variance (data uncertainty), and $\lambda$ is the Levenberg--Marquart regularization parameter.  Our initial guess is given by the column vector $\vb{m}_0$.
