...
\[
    f(\alpha \vb{m}_1) = \alpha f(\vb{m}_1) \, ,
\]
and additivity
\[
    f(\vb{m}_1 + \vb{m}_2) = f(\vb{m}_1) + f(\vb{m}_2) \, ,
\]
where $\alpha$ is a scalar and $\vb{m}_1$ and $\vb{m}_2$ are vectors of model parameters.
...
For instance, $y=ax+b$ is linear with respect to the model parameters $\vb{m}=(a,b)$. For homogeneity,
\begin{align*}
    y(\alpha \vb{m})
        &= (\alpha a)x + (\alpha b) \\
        &= \alpha(ax+b) \\
...
\textbf{Linearity requires additivity and homogeneity.}  Both conditions must hold simultaneously for a function to be linear---satisfying only one is not sufficient. They can be combined into a single linearity test:
\[
    f(\alpha \vb{m}_1 + \beta \vb{m}_2) = \alpha f(\vb{m}_1) + \beta f(\vb{m}_2) \, .
\]
