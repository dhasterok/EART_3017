...
\begin{itemize}
  \item \(\vb{m} \to \) the model is Earth, what it is made of and its physical properties. Changing \(\vb{m}\) changes the Earth.
  \item \(\vb{A} \to \) how Earth talks to the instrument, how the Earth is sampled by the experiment, i.e., the rules of the experiment. Changing \(\vb{A}\) changes the experiment, i.e., same Earth + different experiment \(\to\) different data.
  \item \(\vb{d} \to \) the data is the outcome of the experiment, what we measure
\end{itemize}

In most geophysical problems, \(\vb{A}\) is
\begin{itemize}
  \item gravity -- distances from mass elements
  \item magnetics -- sensor positions and dip angle
...
\end{itemize}
So the entries in \(\vb{A}\) are determined by:
\begin{itemize}
  \item positions
  \item distances
  \item times
...
Put simply, the matrix \(\vb{A}\) encodes the physics and geometry of how the experiment samples the model, \(\vb{m}\), resulting in the observations, \(\vb{d}\).
...
\begin{enumerate}[label=Q\arabic*.,itemsep=6pt]
  ...
  We'll use this formulation in the problem set.  This equation is used  to estimate thermal conductivity model for a solid solution series for a mineral.
  \[
  ...
  The ocean floor records chaotic reversals in Earth's magnetic field.  The remnant magnetic field is ``frozen'' in dikes produced at the mid-ocean ridge.
  \[
...
\textbf{The operator $\vb{A}$ encodes the physics and geometry of the experiment.} Changing the Earth (the model $\vb{m}$) changes the predictions; changing the experiment (the operator $\vb{A}$) also changes the predictions even if the Earth stays the same.

\vspace{0.5em}
\textbf{Linear equations are easier to invert.} Linear equations can be solved in a single step using a linear least-squares inversion (LLSQ) inversion.  Non-linear equations require iterative solutions.

\vspace{0.5em}
\textbf{Many geophysical relations are non-linear but can be linearized.} Power laws become linear under a log-transform; functions with products of unknowns may become linear under a change of variables. Recognizing these opportunities converts a difficult iterative problem into a tractable one.
